...
class CardBrowserTest : RobolectricTest() {
    ...
    fun browserIsNotInitiallyInMultiSelectModeWithNoCards() {
        ...
        assertThat(browser.isInMultiSelectMode, Is.`is`(false))
    }
    ...
    fun selectAllIsNotVisibleWhenNoCardsInDeck() {
        val browser = browserWithNoNewCards
        assertThat(browser.isShowingSelectAll, Is.`is`(false))
    }

    @Test
    fun selectAllIsVisibleWhenCardsInDeck() {
        val browser = browserWithMultipleNotes
        assertThat(browser.cardCount(), Matchers.greaterThan(0L))
        assertThat(browser.isShowingSelectAll, Is.`is`(true))
    }
    ...
    fun selectNoneIsVisibleOnceSelectAllCalled() {
        ...
        selectMenuItem(browser, R.id.action_select_all)
        advanceRobolectricLooperWithSleep()
        assertThat(browser.isShowingSelectNone, Is.`is`(true))
    }
    ...
    fun browserIsInMultiSelectModeWhenSelectingOne() {
        val browser = browserWithMultipleNotes
        selectOneOfManyCards(browser)
        assertThat(browser.isInMultiSelectMode, Is.`is`(true))
    }
    ...
    fun browserDoesNotFailWhenSelectingANonExistingCard() {
        ...
        advanceRobolectricLooperWithSleep()
        assertThat(browser.cardCount(), Matchers.equalTo(5L))
    }
    ...
    fun selectionsAreCorrectWhenNonExistingCardIsRemoved() {
        ...
        // ASSERT
        assertThat(browser.cardCount(), Matchers.equalTo(6L))
        assertThat("A checked card should have been removed", browser.checkedCardCount(), Matchers.equalTo(3))
        assertThat("Checked card before should not have changed", browser.hasCheckedCardAtPosition(1), Is.`is`(true))
        assertThat("Checked card after should have changed by 2 places", browser.hasCheckedCardAtPosition(3), Is.`is`(true))
        assertThat("Checked card after should have changed by 2 places", browser.hasCheckedCardAtPosition(4), Is.`is`(true))
    }
    ...
    fun changeDeckIntegrationTestDynamicAndNon() {
        ...
        val decks = b.validDecksForChangeDeck
        for (d in decks) {
            assertThat(validNames, Matchers.hasItem(d.getString("name")))
        }
        assertThat("Additional unexpected decks were present", decks.size, Is.`is`(2))
    }
    ...
    fun moveToNonDynamicDeckWorks() {
        ...
        for (cardId in cardIds) {
            assertThat("Deck should have been changed yet", col.getCard(cardId).did, Matchers.not(deckIdToChangeTo))
        }
        ...
        advanceRobolectricLooperWithSleep()
        for (cardId in cardIds) {
            assertThat("Deck should be changed", col.getCard(cardId).did, Is.`is`(deckIdToChangeTo))
        }
    }
    ...
    fun changeDeckViaTaskIsHandledCorrectly() {
        ...
        for (cardId in cardIds) {
            assertThat("Deck should not be changed", col.getCard(cardId).did, Matchers.not(dynId))
        }
    }
    ...
    fun flagsAreShownInBigDecksTest() {
        ...
        assertThat("Card should be flagged", getCheckedCard(cardBrowser).card.userFlag(), Is.`is`(flag))
        ...
        assertThat("Card flag should be removed", getCheckedCard(cardBrowser).card.userFlag(), Matchers.not(flag))
    ...
    fun flagValueIsShownOnCard() {
        ...
        assertThat("The card flag value should be reflected in the UI", actualFlag, Is.`is`(1))
    }
    ...
    fun startupFromCardBrowserActionItemShouldEndActivityIfNoPermissions() {
        ...
        assertThat(component, Matchers.notNullValue())
        val componentName = component!!
        assertThat("Deck Picker currently handles permissions, so should be called", componentName.className, Is.`is`("com.ichi2.anki.DeckPicker"))
        assertThat("Activity should be finishing", cardBrowser.isFinishing)
        assertThat("Activity should be cancelled as it did nothing", shadowActivity.resultCode, Is.`is`(Activity.RESULT_CANCELED))
    }
    ...
    fun tagWithBracketsDisplaysProperly() {
        ...
        advanceRobolectricLooperWithSleep()
        assertThat("tagged card should be returned", b.cardCount, Is.`is`(1))
    }
    ...
    fun filterByFlagDisplaysProperly() {
        ...
        assertThat("Flagged cards should be returned", b.cardCount, Is.`is`(2))
    }
    ...
    fun previewWorksAfterSort() {
        ...
        assertThat(b.getPropertiesForCardId(cid1).position, Is.`is`(0))
        assertThat(b.getPropertiesForCardId(cid2).position, Is.`is`(1))

        b.checkCardsAtPositions(0)
        val previewIntent = b.previewIntent
        assertThat("before: index", previewIntent.getIntExtra("index", -100), Is.`is`(0))
        assertThat("before: cards", previewIntent.getLongArrayExtra("cardList"), Is.`is`(longArrayOf(cid1, cid2)))

        // reverse
        b.changeCardOrder(1)

        assertThat(b.getPropertiesForCardId(cid1).position, Is.`is`(1))
        assertThat(b.getPropertiesForCardId(cid2).position, Is.`is`(0))

        b.replaceSelectionWith(intArrayOf(0))
        val intentAfterReverse = b.previewIntent
        assertThat("after: index", intentAfterReverse.getIntExtra("index", -100), Is.`is`(0))
        assertThat("after: cards", intentAfterReverse.getLongArrayExtra("cardList"), Is.`is`(longArrayOf(cid2, cid1)))
    }
    ...
    fun addCardDeckIsNotSetIfAllDecksSelectedAfterLoad() {
        addDeck("NotDefault")
        val b = browserWithNoNewCards
        assertThat("All decks should not be selected", b.hasSelectedAllDecks(), Is.`is`(false))
        b.selectAllDecks()
        assertThat("All decks should be selected", b.hasSelectedAllDecks(), Is.`is`(true))
    ...
    fun addCardDeckISetIfDeckIsSelected() {
        ...
        val b = browserWithNoNewCards
        assertThat("The target deck should not yet be selected", b.lastDeckId, Matchers.not(Is.`is`(targetDid)))
        b.selectDeckAndSave(targetDid)
        assertThat("The target deck should be selected", b.lastDeckId, Is.`is`(targetDid))
        val addIntent = b.addNoteIntent
    ...
    fun addCardDeckISetIfDeckIsSelectedOnOpen() {
        ...
        val b = browserWithNoNewCards
        assertThat("The initial deck should be selected", b.lastDeckId, Is.`is`(initialDid))
        val addIntent = b.addNoteIntent
    ...
    fun repositionDataTest() {
        ...
        assertThat("Initial position of checked card", card.getColumnHeaderText(CardBrowser.Column.DUE), Is.`is`("1"))
        b.repositionCardsNoValidation(listOf(card.id), 2)
        advanceRobolectricLooperWithSleep()
        assertThat("Position of checked card after reposition", card.getColumnHeaderText(CardBrowser.Column.DUE), Is.`is`("2"))
    }
    ...
    fun resetDataTest() {
        ...
        b.checkCardsAtPositions(0)
        val card = getCheckedCard(b)
        assertThat("Initial due of checked card", card.getColumnHeaderText(CardBrowser.Column.DUE), Is.`is`("8/12/20"))
        b.resetProgressNoConfirm(listOf(card.id))
        advanceRobolectricLooperWithSleep()
        assertThat("Position of checked card after reset", card.getColumnHeaderText(CardBrowser.Column.DUE), Is.`is`("1"))
    }
    ...
    fun rescheduleDataTest() {
        ...
        advanceRobolectricLooperWithSleep()
        assertThat("Due of checked card after reschedule", card.getColumnHeaderText(CardBrowser.Column.DUE), Is.`is`("8/12/20"))
    }
    ...
    fun dataUpdatesAfterUndoReposition() {
        ...
        advanceRobolectricLooperWithSleep()
        advanceRobolectricLooperWithSleep()
        assertThat("Position of checked card after undo should be reset", card.getColumnHeaderText(CardBrowser.Column.DUE), Is.`is`("1"))
    }
    ...
    fun checkSearchString() {
        ...
        advanceRobolectricLooperWithSleep()
        assertThat("Cardbrowser has Deck 1 as selected deck", cardBrowser.selectedDeckNameForUi, Is.`is`("Deck 1"))
        assertThat("Results should only be from the selected deck", cardBrowser.cardCount, Is.`is`(1))
    }
    ...
    fun checkDisplayOrderPersistence() {
        ...
        assertThat("Initially Card Browser has order = noteFld", col.get_config_string("sortType"), Is.`is`("noteFld"))
        ...
        assertThat("Card Browser has the new sortType field", col.get_config_string("sortType"), Is.`is`("cardEase"))
    ...
    fun checkIfLongSelectChecksAllCardsInBetween() {
        ...
        assertThat(browser.checkedCardCount(), Is.`is`(18))
    }
    ...
    fun checkIfSearchAllDecksWorks() {
        ...
        advanceRobolectricLooperWithSleep()
        assertThat("Card browser should have Test Deck as the selected deck", cardBrowser.selectedDeckNameForUi, Is.`is`("Test Deck"))
        assertThat("Result should be empty", cardBrowser.cardCount, Is.`is`(0))
        cardBrowser.searchAllDecks()
        advanceRobolectricLooperWithSleep()
        assertThat("Result should contain one card", cardBrowser.cardCount, Is.`is`(1))
    }
    ...
    protected fun assertUndoDoesNotContain(browser: CardBrowser, @StringRes resId: Int) {
        ...
        assertThat(item.title.toString(), Matchers.not(Matchers.containsString(expected.lowercase(Locale.getDefault()))))
    }
    ...
    protected fun assertUndoContains(browser: CardBrowser, @StringRes resId: Int) {
        ...
        val item = shadowActivity.optionsMenu.findItem(R.id.action_undo)
        val expected = browser.getString(resId)
        assertThat(item.title.toString(), Matchers.containsString(expected.lowercase(Locale.getDefault())))
    }

    private fun getCheckedCard(b: CardBrowser): CardCache {
        val ids = b.checkedCardIds
        assertThat("only one card expected to be checked", ids, Matchers.hasSize(1))

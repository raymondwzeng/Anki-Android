...
class CardBrowser : NavigationDrawerActivity(), SubtitleListener, DeckSelectionListener, TagsDialogListener {
    ...
    var onEditCardActivityResult = registerForActivityResult<Intent, ActivityResult>(ActivityResultContracts.StartActivityForResult()) { result: ActivityResult ->
    ...
    var onAddNoteActivityResult = registerForActivityResult<Intent, ActivityResult>(ActivityResultContracts.StartActivityForResult()) { result: ActivityResult ->
    ...
    var onPreviewCardsActivityResult = registerForActivityResult<Intent, ActivityResult>(ActivityResultContracts.StartActivityForResult()) { result: ActivityResult ->
    ...
    protected fun changeCardOrder(which: Int) {
        if (which != mOrder) {
            mOrder = which
            mOrderAsc = false
            if (mOrder == 0) {
    ...
    // TODO: Ask about whether or not these methods should support nullable cardbrowser?
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (intent.getStringExtra("search_query") != null) {
            // Odd compiler issue, even though we check for null in if statement, still needs !!
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        ...
        if (itemId == android.R.id.home) {
            ...
            val truncateLinesDialog = IntegerDialog()
            ...
            truncateLinesDialog.setCallbackRunnable { lines: Int -> changesLinesTruncated(lines) }
            showDialogFragment(truncateLinesDialog)
        }
        return super.onOptionsItemSelected(item)
    }

    protected fun changesLinesTruncated(linesToTruncate: Int) {
        for (cardIndex in 0 until mCardsAdapter!!.count) {
            val cardView = mCardsAdapter!!.getView(cardIndex, null, null)
            val sfldView: FixedTextView = cardView.findViewById(R.id.card_sfld)
            val column2View: FixedTextView = cardView.findViewById(R.id.card_column2)
            sfldView.maxLines = linesToTruncate
            column2View.maxLines = linesToTruncate
        }
    }
    ...
    fun onUndo() {
        if (col.undoAvailable()) {
            Undo().runWithHandler(mUndoHandler) // TODO: Figure out why
        }
    }
    ...
    override fun onSelectedTags(selectedTags: List<String>, indeterminateTags: List<String>, option: Int) {
        when (mTagsDialogListenerAction) {
            ...
            else -> {} // TODO: Fill something here, or maybe not?
        }
    }
    ...
    // TODO: Potential unwanted exposure of class, might need a second review.
    abstract class ListenerWithProgressBar<Progress, Result>(browser: CardBrowser?) : TaskListenerWithContext<CardBrowser?, Progress, Result>(browser) {
        override fun actualOnPreExecute(context: CardBrowser?) {
    ...
    private abstract class ListenerWithProgressBarCloseOnFalse<Progress, Result : Computation<*>>(private var timber: String?, browser: CardBrowser?) : ListenerWithProgressBar<Progress, Result>(browser) {
     ...
     */
    private fun updateCardsInList(cardList: List<Card>) {
        val cardCollection: CardCollection<CardCache> = cards // Renamed because Kotlin was getting the wrong one.
        val idToPos = getPositionMap(cardCollection)
        for (c in cardList) {
            ...
            // update Q & A etc
            cardCollection[pos].load(true, mColumn1Index, mColumn2Index)
        }
        updateList()
    }
    ...
    private val mSearchCardsHandler: SearchCardsHandler = SearchCardsHandler(this)
    ...
    internal inner class SearchCardsHandler(browser: CardBrowser?) : ListenerWithProgressBar<List<CardCache>, SearchCardsResult>(browser) {
        ...
        override fun actualOnPostExecute(context: CardBrowser?, result: SearchCardsResult) {
            if (result.hasResult) {
                mCards.replaceWith(result.result as MutableList<CardCache>?)
                updateList()
                handleSearchResult()
            }
            if (result.hasError) {
    ...
    private fun onSelectionChanged() {
        ...
        try {
            ...
            if (!isInMultiSelectMode && mActionBarMenu != null) {
                ...
                selectAll.isVisible = cardCount() != 0L
            }
            if (!isInMultiSelectMode) {
                return
            }
    ...
    class CardCollection<T : PositionAware> : Iterable<T> {
        private var mWrapped: MutableList<T>? = ArrayList()
        fun size(): Int {
    ...
    class CardCache : Card.Cache, PositionAware {
        ...
        fun load(reload: Boolean, column1Index: Int, column2Index: Int) {
            ...
            card.note()
            if (COLUMN1_KEYS[column1Index] == CardBrowser.Column.QUESTION || COLUMN2_KEYS[column2Index] == CardBrowser.Column.QUESTION || COLUMN2_KEYS[column2Index] == CardBrowser.Column.ANSWER // First column can not be the answer. If it were to
    ...
    @VisibleForTesting(otherwise = VisibleForTesting.NONE)
    fun // should only be called from changeDeck()
    executeChangeCollectionTask(ids: List<Long>?, newDid: Long) {
    ...
    companion object {
        ...
        private val COLUMN1_KEYS = arrayOf<Column>(CardBrowser.Column.QUESTION, CardBrowser.Column.SFLD)
